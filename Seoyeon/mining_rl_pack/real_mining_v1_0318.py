...
보상 구조 (3-Layer):
  Layer 1  Y-level shaping   — 깊이에 비례한 연속 보상 (지표→Y=-58 안내)
  Layer 2  탐험 보너스 (λ)   — 새 위치 방문 시 보너스, 학습 진행에 따라 감쇠
  Layer 3  광물 채굴 (sparse) — 인벤토리 델타 기반, 다이아몬드 +100 등
"""
...
STEP_PENALTY       = -0.02    # 매 스텝 패널티 (↑ 0.01→0.02, 게으름 방지 강화)
DEATH_PENALTY      = -30.0    # 사망 패널티 (광물 보상 스케일에 맞춤)
HEALTH_LOSS_K      =  -0.2    # 체력 피해 스텝당 패널티 계수
BLOCK_BREAK_BONUS  =  0.15    # 아무 블록이든 파괴 시 보상 (↑ 0.05→0.15, 채굴 행동 강화)

ORE_AIM_BONUS        = 0.3     # 광물 블록을 조준(시선)만 해도 보상
ORE_AIM_ATTACK_BONUS = 1.0    # 광물 블록을 조준하고 공격 시 추가 보상
LAYER1_MAX         =   0.15   # Y-level shaping 최대 보상/스텝 (↑ 0.10→0.15, 깊이 유도 강화)
Y_DELTA_BONUS      =   0.5    # Y가 이전 최저점보다 낮아질 때 보상 (더 깊이 파는 행동)
LAYER2_BASE        =   0.50   # 새 셀 탐험 기본 보너스
EXPLORE_CELL_SIZE  =   2      # 탐험 셀 크기 (↓ 4→2, 터널 채굴에 적합한 세밀한 탐색)
...
class MiningWrapper(gym.Wrapper):
    ...
    Obs: {
        ...
        "state":  (10,) float32     — 깊이·체력·방향·λ·광물조준
    }
    Act: Discrete(13)

    주요 설계:
      - Layer 1: Y-level shaping → 에이전트를 Y=-58 방향으로 유도
      - Layer 2: 탐험 보너스 (λ 감쇠) → 지하 공간 탐색 유도
      - Layer 3: 인벤토리 델타 → 광물 채굴 시 sparse 보상
    """
    ...
    def __init__(
        ...
        # lambda_intrinsic: _compute_reward 에서 매 스텝 자체 갱신
        # set_attr로 외부에서 덮어쓸 수 있음 (LambdaDecayCallback 사용 시)
    ...
    def reset(self, **kwargs):
        ...
        # clear @p 이후 인벤토리 기준선 확립 (ore 아이템 0개여야 함)
    ...
    def _compute_reward(
        ...
        # ── Layer 1: Y-level shaping ──────────────────────────────
        # 지표(Y=64)에서 0, 다이아 층(Y=-58)에서 최대 LAYER1_MAX
        y = _scalar(raw, "y")
        depth_progress = float(np.clip((Y_SURFACE - y) / Y_RANGE, 0.0, 1.0))
        r += LAYER1_MAX * depth_progress

        # ── Layer 1b: Y-delta 보상 (더 깊이 파는 행동 즉시 보상) ───
        if y < self._min_y:
            delta_y = self._min_y - y  # 양수: 얼마나 더 깊이 갔는지
            r += Y_DELTA_BONUS * delta_y
            self._min_y = y

        # ── Layer 1c: 깊이 마일스톤 보너스 (1회성) ──────────────
        ...
        x = _scalar(raw, "x")
        z = _scalar(raw, "z")
        # 2-block 단위 셀로 해시 (↓ 4→2, 터널 채굴에 적합)
        cs = EXPLORE_CELL_SIZE
        cell = (int(x) // cs, int(y) // cs, int(z) // cs)
        if cell not in self._visited:
            self._visited.add(cell)
            # 깊을수록 탐험 가치 증가: depth_progress=0(지표)→1.0, depth=1(Y=-58)→2.0
            depth_weight = 1.0 + depth_progress
            r += self.lambda_intrinsic * LAYER2_BASE * depth_weight

        # ── Layer 3a: 광물 조준 보상 (시선 유도) ─────────────────
        # 광물을 바라보기만 해도 소량 보상 → 시선 탐색 강화
        # 공격까지 하면 추가 보상 → 조준→공격 행동 체인 유도
        hit = _get_hit(raw)
        if hit is not None and _hit_type(hit) == "block":
            block_id = _strip_state(_hit_state(hit))
            if block_id in ORE_BLOCK_IDS:
                ore_val = _ORE_BLOCK_VALUE.get(block_id, 1.0) / 100.0
                r += ORE_AIM_BONUS * ore_val          # 조준만으로 보상
                if is_attack:
                    r += ORE_AIM_ATTACK_BONUS * ore_val  # 공격 시 추가

        # ── Layer 3b: 블록 파괴 보상 ─────────────────────────────
        # 인벤토리 총량 증가 = 블록을 파괴해서 아이템 획득
        # 광물이 아닌 돌/딥슬레이트도 포함 → 채굴 행동 자체를 강화
        prev_total = sum(self._prev_inv.values())
        curr_total = sum(curr_inv.values())
        if curr_total > prev_total:
            r += BLOCK_BREAK_BONUS

        # ── Layer 3c: 광물 채굴 보상 (인벤토리 델타, log 압축) ───
        # log(1 + gained) 로 다수 드롭 시 보상을 압축하여 분산 축소
        #   Fortune III 다이아 4개: 20 × log(1+4) = 20 × 1.61 = 32.2
        #   Fortune III 다이아 1개: 20 × log(1+1) = 20 × 0.69 = 13.9
        #   → 4배 드롭 차이가 보상에선 2.3배로 압축됨
    ...
    def _render(self, img_rgb: np.ndarray | None) -> np.ndarray:
        ...
        y   = -99.0  # placeholder (실제 y는 step()에서 전달하지 않음)
        lam = f"{self.lambda_intrinsic:.2f}"
        lines = [
            f"Step:{self._step:4d}  R:{self._ep_r:+7.1f}  λ:{lam}",
            f"Mode:{self.mode_name.upper()}",
        ]
...
class MiningCNNExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, obs_space: gym.spaces.Dict, features_dim: int = 256):
        ...
        state_dim = obs_space["state"].shape[0]  # 10
...
class MiningTrackingCallback(BaseCallback):
    ...
    def _log(self):
        ...
        # ep_info_buffer: VecMonitor가 채운 에피소드 완료 기록
...
class RenderCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if frame is None:
            frame = np.zeros((MiningWrapper.H * 3, MiningWrapper.W * 3, 3), dtype=np.uint8)
        try:
